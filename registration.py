...
def full_registration(pcds_down_standard: List[o3d.geometry.PointCloud], voxel_size: float, max_correspondence_distance: float, 
    ...
    Args:
        ...
        initial_poses: Optional list of initial poses for odometry initialization
        min_fitness_consecutive: Minimum fitness for consecutive frame registration
    ...
    print(f"\nPairwise registration for {n_pcds} point clouds (fragment/local)...")
    ...
    for source_id in range(n_pcds - 1):
        ...
        if transformation is not None and result is not None:
            ...
            if fitness >= min_fitness_consecutive and is_valid_transformation(transformation):
                ...
                print(f"✗ fitness={fitness:.3f} (low/invalid)") if verbose else None
            ...
            print("✗ registration failed") if verbose else None
            consecutive_failed += 1
            transformation = np.identity(4)
            odometry = np.dot(transformation, odometry)
            pose_graph.nodes.append(
    ...
    for source_id in range(0, n_pcds, loop_closure_interval):
        for target_id in range(source_id + loop_closure_distance_threshold, 
                              n_pcds, loop_closure_interval):
            if target_id >= n_pcds:
                break
            loop_attempted += 1
            frame_distance = target_id - source_id
            print(f"  Loop {source_id} → {target_id} (distance={frame_distance})", end=" ") if verbose else None
            ...
            if transformation is not None and result is not None:
                fitness = result.fitness
                if fitness >= min_fitness_loop and is_valid_transformation(
    ...
    print(f"\n✓ Local registration complete: {consecutive_success} consecutive OK, "
